...
class PackageSubtypeAnalysis(lps: LayerModuleStructure) {
  ...
  class CommonSubtypeDep(t: ModuleNode, es: List[DepEdge]) {
    ...
    val from = es.map(_.getSource).distinct
    ...
    private def analyze = {
      ...
      for (st <- stmap.keys) {
        if (stmap(st).length > 1) {
          ...
          if (modules.filter(_ != to.asInstanceOf[LayerModuleNode].pkgname).distinct.length > 0) {
            heteroPackages = true
    ...
    override def toString = t + "<-- " + from.map(m => m.asInstanceOf[LayerModuleNode].pkgname).distinct + " : " + from + " \n" +
  ...
  def getInterPackageSubtypings: List[ModuleEdge] = {
    ...
    for (n <- lpstruct.nodes.values) {
      ...
      println("------------------")
      if (stedges2n.map(_.getSource).distinct.length > 1) {
