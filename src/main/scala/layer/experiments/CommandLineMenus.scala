...
object CommandLineMenus {
  ...
  private def select(options: List[String]): Int = {
    ...
    try {
      ...
      case e: IOException => {
        System.out.println("IO error trying to read your choice!"); System.exit(1);
      }
    }
    selected
  }
  ...
  def selectProject: String = {
    val input = List("android", "argouml", "awt", "layer/bcel", "findbugs", "hadoop", "hibernate", "jfreechart", "jhotdraw6",
