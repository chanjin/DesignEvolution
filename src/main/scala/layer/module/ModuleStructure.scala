...
class ModuleEdge(m1: ModuleNode, m2: ModuleNode) extends DepEdge(m1, m2) {
  ...
  override def toDot: String = {
    ...
    edgeCount + ":" + subtypeEdgeCount + ":" + typeedges.map(_.getTarget).distinct.length // +
...
class ModuleStructure(g: TypeStructure) extends DependencyGraph {
  ...
  def getEdgesUnderThreshold(): List[DepEdge] = {
    ...
    // threshold (x - m) / sigma < - 1.645 (lower 0.5%), x < m - 1.645 * sigma�� �����ϴ� �ִ� ����
  ...
  def toTable = {
    val modules = nodes.values.toList.sortWith((m1, m2) => m1.value.compareTo(m2.value) < 0)
    ...
    modules.map(m => {
      ...
      name + "\t" + numTypes + "\t" + typeList.toList.sortWith((t1, t2) => t1.compareTo(t2) < 0).mkString(", ")
  ...
  def getModules(ms: List[T]): List[T] = {
    ms.foldLeft(List[T]())((result, n) =>
      if (n.isInstanceOf[CompositeModuleNode[T]])
        n.asInstanceOf[CompositeModuleNode[T]].modules ::: result
      else n :: result);
  }

  def addModule(n: T) = {
    modules = if (n.isInstanceOf[CompositeModuleNode[T]])
      n.asInstanceOf[CompositeModuleNode[T]].modules ::: modules
    else n :: modules
  }
