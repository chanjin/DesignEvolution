...
class LayerModuleContainerNode(qn: String, r: Int, pkg: String, ms: List[LayerModuleNode])
  ...
  override def toDot = {
    def suffixes = modules.filter(_.value != this.value).sortWith(_.rank < _.rank).map(m =>
...
class LayerModuleStructure(tdg: TypeStructure) extends ModuleStructure(tdg) {
  ...
  def printLayerDetails = {
    println("----------------")
    nodes.values.toList.sortWith((n1, n2) => n1.toString.compareTo(n2.toString) < 0).foreach(n => {
      print("** " + n.toString + ": ")
      println(n.asInstanceOf[LayerModuleNode].types.sortWith((t1, t2) => t1.value.compareTo(t2.value) < 0).
