...
object Graph2File {
  ...
  def toFileType(g: DependencyGraph, dotfilename: String, nodelist: List[NodeElem]): Unit = {
    ...
    val edgeStrs = g.edges.filter(edgefilter(_)).sortWith((e1, e2) => e1.getSource.value.compareTo(e2.getSource.value) < 0).
  ...
  def toFileModule(g: ModuleStructure, dotfilename: String, nodelist: List[NodeElem]): Unit = {
    ...
    val nodes = g.nodes
    val edges = g.edges
    val mns = nodes.values.toList.filter(nodelist.contains(_)).sortWith(_.value < _.value)
    val mnsDetails = mns.sortWith(_.value < _.value).map(n => {
      val mn = n.asInstanceOf[ModuleNode]
      val types = mn.types.filter(!_.value.contains("$")).sortWith(_.value < _.value).toList
      val typesInner = mn.types.filter(_.value.contains("$")).sortWith(_.value < _.value).toList
    ...
    val mnsStr = mns.map(n => {
      n.outedges.sortWith(_.getSource.value < _.getSource.value).
        map(e => (e.getSource.value + "\t" + e.getTarget.value)).filter(_.length != 0).mkString("\n")
    })
    ...
    val mnsStrDetails = mns.map(n => {
      ...
      n.outedges.sortWith(_.getSource.value < _.getSource.value).map(e => {
        val dstprefix = e.getTarget.asInstanceOf[ModuleNode].packageName

        "***" + e.getSource.value + "->" + e.getTarget.value + "\n" +
          e.asInstanceOf[ModuleEdge].getTypeEdges.sortWith(_.getSource.value < _.getSource.value)
