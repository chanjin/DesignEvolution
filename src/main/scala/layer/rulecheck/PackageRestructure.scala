...
import layer.configuration.ProjectConfiguration
...
object PackageRestructure {
  ...
  def makeNewPackageStructure(projconf: ProjectConfiguration, pdg: PackageStructure, lps: LayerModuleStructure): PackageStructure = {
    ...
    val newtdg = if (projconf.ignores.size > 0) {
      ...
      println("FILTERED nodes: " + refEdges.map(_.getTarget).distinct.size + "\n" +
        refEdges.map(_.getTarget).distinct.mkString("\n"))

      println("FILTERED nodes (source): " + refEdges.map(_.getSource).distinct.size + "\n" +
        refEdges.map(_.getSource).distinct.mkString("\n"))
