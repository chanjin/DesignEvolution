...
class CycleCheck {
  ...
  def checkPackageCycles(dotprefix: String) = {
    pdg.detectSCC.foreach(ms => {
      val pkgs = ms.map(mn =>
        Graph2Dot.dotNamePackage(mn.value)).sortWith((m1, m2) => m1.toString.compareTo(m2.toString) < 0)
  ...
  def checkPackageDependencyViolaitons(violations: Map[String, List[String]]): List[DepEdge] = {
    ...
    println("Violating edges: " + vedgesall.distinct.length)
    val vnodes = vedgesall.map(_.getTarget).distinct
