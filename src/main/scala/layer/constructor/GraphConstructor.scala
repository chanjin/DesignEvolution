...
object GraphConstructor {
    ...
    def constructTypeStructure(path: String): (TypeStructure, PackageStructure) = {
        ...
        for (clazz <- types) {
            val structuraldeps = ClassInformation.getStructuralDependents(clazz).filter(t => nodes2analyze.contains(t)).distinct
            ...
            val cpdeps = ClassInformation.getConstPoolClasses(clazz).filter(t => nodes2analyze.contains(t)).distinct
            val dependents = (structuraldeps ::: cpdeps).distinct
            val ancestors = ClassInformation.getAncestors(clazz).filter(ancestor => nodes2analyze.contains(ancestor.getClassName)).distinct
